...
| S13 | **`CITATION.cff` keywords** (7 generic) | Zenodo, Scholar | generic | add: `reverse-engineering`, `stack-trace-mapping`, `model-context-protocol`, `AI-assisted-debugging`. **Correction (2026-09-12)**: this row said `version` / `date-released` were Zenodo-archive-pinned and must not change. They are pinned *to the archive*, and Zenodo's GitHub integration archives every GitHub Release automatically — so they must be **updated after each release** to match the record the concept DOI now resolves to. Verify against the Zenodo API rather than assuming; the release checklist in `CLAUDE.md` carries the commands | No (docs-only) |
...
**S15 social preview — completed 2026-09-13.** The scan missed a repo-surface
item: with no Open Graph image, every link to the repository posted anywhere
rendered GitHub's generic auto-generated card. The card is now composed by
`scripts/generate_social_preview.py` from the existing shield logo, so it
cannot drift from the mark used in the README and on the docs site, and
rebuilding it after a tagline change is one command. The script refuses to
write a card whose text overflows the safe area or exceeds GitHub's 1 MB
limit. Uploaded by the maintainer and verified live: 1280x640 PNG, 112 KB,
served from `repository-images.githubusercontent.com`.

Deliberately not generated by an image model: the payload of a social card is
its text, and a misspelt product name is worse than no image at all.
