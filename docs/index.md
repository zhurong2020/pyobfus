...
<div style="background: #f8f9fa; padding: 1.5em; border-radius: 8px; border-left: 4px solid #27ae60;">
  ...
  <ul style="color: #555; line-height: 1.8; margin-bottom: 0;">
    ...
    <li><strong>Numeric / Constant Obfuscation</strong> (<code>--numeric-obfuscation</code>): replace integer/float literals with value-preserving opaque expressions</li>
    <li><strong>AI Provenance Stripping</strong> (<code>--strip-ai-artifacts</code>): remove <code>Generated by Claude</code> / <code>Co-Authored-By</code> markers from docstrings and attribution dunders</li>
    ...
    <li><strong>Reproducible Builds</strong> (next release): identical input and configuration produce identical output bytes, so a recipient can rebuild and check the digests in <code>--build-report</code>; randomised transforms (<code>--numeric-obfuscation</code>, string encryption) are excluded by design</li>
    <li><strong>Verifiable Build Report</strong> (v0.5.24): <code>--build-report</code> writes deterministic, privacy-safe JSON facts connecting selection/config, transformations, verification, output hashes, artifacts, markers, and provenance</li>
    <li><strong>GitHub Action</strong>: <code>uses: zhurong2020/pyobfus-action@v1</code> runs the scan or a build in CI with SARIF for Code Scanning, separating findings from tool errors</li>
...
- [Provenance Manifest](PROVENANCE_MANIFEST.md) - Obfuscated-output manifest fields and integrity notes
- [Release Provenance](RELEASE_PROVENANCE_VERIFICATION.md) - PyPI Integrity API and attestation verification runbook
