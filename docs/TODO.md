...
最后更新：2026-09-13（P0 许可系统可靠性四项已全部发布为 `0.5.26`，去处见 `CURRENT_PLAN_ZH.md`）。
...
- `pyobfus_mcp/CHANGELOG.md` 的 `[Unreleased]` 有 mcp SDK 2.x 兼容一条。**没有随
  0.5.26 一起发**，因为那是 Core 的紧急修复，不该捆绑无关内容；且每发一次 MCP
  都要手工改 Glama 的 Build steps。攒够增量或有人明确要 2.x 时再发。
