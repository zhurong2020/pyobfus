...
## P0 · 许可系统可靠性 —— 代码已全部完成，只剩两道 gate

背景：2026-09-12 一位新客户报激活失败，查出 **Pro 联网激活对所有客户全线失效**，
最后一次成功验证停在 2026-07-08，无人察觉近两个月。根因是 Cloudflare 边缘拦
`urllib` 默认 UA，但真正让它变成收入损失的是两件设计缺陷：许可系统对付费客户
**fail closed**，并且**没有任何监控**。排查中又查出三件更早的缺陷（见 CHANGELOG
`[Unreleased]`）。完整档案见 memory `pyobfus_license_edge_block_2026-09-12`。

四项实现均已 commit，四个测试根 + Worker 测试 + CI 原样的 black/ruff/mypy 全绿。
**剩下的只有两件事，都需要用户批准，且有先后顺序：**

| 顺序 | 动作 | 为什么不能颠倒 |
|---|---|---|
| 1 | **部署 Worker**（`npx wrangler deploy`）+ `wrangler secret put ADMIN_TOKEN` | 新客户端会调 `/api/deactivate`；先发客户端会让 `deactivate` 对着没有该路由的服务器跑 |
| 2 | **发 `0.5.26`** | 见下 |

部署前先导出 KV 现状（唯一副本，写坏不可恢复）。`ADMIN_TOKEN` 用独立随机串，
**不要**复用 Cloudflare API token。

发版验收与平时不同：全新 venv 装完后要**真的对生产端点跑一次激活**（这个版本修
的就是这条路径），用用户本人序列号，取用见 memory
`reference-owner-pro-license-key`。GitHub Release 说明里必须写上 `--no-verify`,
因为升级只救新安装，已装机器看不到修复。

发版后把合成监控的 workflow 手动触发一次确认在 CI 环境也绿
（`.github/workflows/license-endpoint-monitor.yml`，每天 06:00/18:00 UTC 自动跑；
已实测：正常时绿，复刻旧 UA 故障时红，端点不可达时红）。

**还没做、留作后续**：`deactivate` 的服务端记录已带审计字段，但 KV 仍无定期导出。
「改动前先导出」目前只是约定，不是机制。
