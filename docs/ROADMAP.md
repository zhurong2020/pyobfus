...
- [~] **P2-12: `pyobfus-mcp` `scan_secrets` tool** — _Re-audited 2026-08-04: partially shipped, folded into an existing tool rather than standalone._ `check_obfuscation_risks`'s `pro_value` field (`tools.py` `_SENSITIVE_LITERAL_PATTERNS` / `_count_sensitive_literals` / `_pro_value_for_scan`) already detects API-key/secret/bearer/password assignment shapes, Stripe live/test keys, AWS access-key IDs, and generic 40+-char opaque tokens, returning a structured Pro recommendation. **Still missing**: the PII-shape patterns this item specifically asked for (emails / IPv4 / GUIDs / filesystem paths — a different signal class than credential shapes), and it isn't exposed as its own dedicated `scan_secrets` tool a client could call directly. Remaining scope is smaller than the original estimate: extend the existing pattern list, and decide (design call, not mechanical) whether to keep it folded into `check_obfuscation_risks` or split into a standalone tool (the latter also changes the MCP tool count, which touches Glama/Registry surfaces). _Revised estimate: 1-2 days._
...
- [~] **P2-15: Anti-debug guard (Pro, opt-in)** — _Re-audited 2026-08-04: partially shipped._ `--anti-debug` already exists and is fully wired (`pyobfus_pro/anti_debug.py`'s `AntiDebugInjector`, default OFF), but today it only implements `sys.gettrace()` detection (catches Python-level debuggers/tracers). **Still missing**: the native-level checks this item specifically asked for — TracerPid (Linux) / IsDebuggerPresent (Windows) / timing-skew — which catch OS-level debugger attachment that `sys.gettrace()` alone can't see. Remaining scope is an enhancement to the existing injector, not a new feature. _Revised estimate: 2-3 days._
