...
- [~] **P2-18: LLM-deobfuscation-resistance mode + benchmark** — the benchmark harness, five-sample corpus, functional scorer, reproducibility metadata, locked-down real-output executor, and blocking offline CI smoke job are implemented. **First real (non-stub) pilot ran 2026-08-01** — Codex CLI / `gpt-5.6-sol`, 2 of 5 corpus samples, all 6 conditions; it also surfaced and fixed a corpus-quality bug (public-domain-algorithm samples let an attacker "recall" the answer instead of defeating the obfuscation, and a no-op transform was silently diluting an aggregate) — see `docs/POST_V0.4_TODO.md` § P2-18 for the fix and the real numbers. A larger corpus and a second model family still remain before a *reviewed public* result; stub output is still not evidence. A `--llm-resistant` preset stays deferred until the benchmark justifies it. _Estimate: measurement + review 1 day; mode 1-2 weeks if justified_
...
**Last Updated**: 2026-08-01 — P2-18 got its first real (non-stub) pilot run (Codex CLI / `gpt-5.6-sol`, 2 of 5 corpus samples, all 6 conditions), which surfaced and fixed a corpus-quality bug (public-domain-algorithm samples + a silently-diluting no-op transform); see `docs/POST_V0.4_TODO.md` for the fix and real numbers.
