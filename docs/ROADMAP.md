...
This is the first periodic review after the 2026-08-04 to 2026-08-07
release wave, covering download trend, support surface, external platform
state, a fresh competitor scan, and a retrospective on the shipped design.

**Health and distribution signal**: GitHub is quiet and healthy: open
issues/PRs are empty, and the latest `main` CI + CodeQL runs on commit
`911ce27` are both successful. pypistats is still noisy in the expected
post-release way rather than showing clean organic growth: same-session
snapshot for `pyobfus` was day/week/month **34 / 1,093 / 1,871**; the
fresh `pyobfus-mcp` endpoint returned **9 / 462 / 601**. The `pyobfus`
endpoint itself hit a 429 during this review, so keep the earlier
same-day snapshot as the recorded value rather than inventing a fresh one.
Interpretation: daily counts have fallen back from the 08-06 release/CI
spike, while week/month remain elevated by recent release traffic.

**External blockers**: Glama remains the active blocker. The public API
still returns `tools: []` for `zhurong2020/pyobfus`, even though the
2026-08-08 manual Build & Release
`019fe034-7da2-73ef-8b02-b279c9ae4b68` succeeded and Glama's own logs saw
all 8 tools during `initialize -> ListToolsRequest`. Treat this as a
Glama release-to-directory-public-API sync bug, not a pyobfus code/config
problem; next action is waiting on / following up in Glama Discord
`#support`. Claude plugin marketplace submission is still pending review
since Aug 2; the public `claude-plugins-community` marketplace has no
`pyobfus`/`protect_project` match, and the submitted `protected_project`
typo remains an opportunistic fix only if Anthropic gives an edit path or
asks for changes.

**Fresh competitor scan**: PyArmor's public docs still show 9.2.6 and keep
moving in the bytecode/native/runtime lane (`rft`, `mini`, `vmc`, `ecc`,
BCC/JIT/Themida-style switches), which is strategically separate from
pyobfus's AST/source-preserving, framework-aware lane. Nuitka's official
material now shows the active 4.1 series with 4.2 upcoming, but its
Commercial traceback encryption page still describes symmetric encryption
with asymmetric support planned; P2-23 remains accurate and the comparison
link was updated to the current official page path. VS Code Marketplace
still has no direct first-party PyArmor/Nuitka/Sourcedefender-style Python
obfuscation extension, but the "no related extensions" claim should be
kept narrower: adjacent tools exist for generic text obfuscation, encrypted
files/folders plus Cython compilation, and integrity signing. That validates
our emphasis on project diagnostics, reverse trace mapping, config
IntelliSense, and verified pyobfus CLI contracts rather than generic
"encrypt this file" features. PyPI still has small AST entrants such as
`python-obfuscator` and `obfus`, but no observed combination of
framework-aware presets, reverse stack-trace mapping, MCP tools, VS Code
workflow, and trust/provenance surfaces.

**Design retrospective**: The VS Code extension's diagnostic-first design
was the right call; after publication it still looks more defensible than a
right-click-only obfuscator. `pyobfus/config_schema.py` is now the pattern to
reuse for future schemas/manifests because it killed a real drift bug instead
of adding another hand-maintained list. Folding PII-shape detection into the
existing MCP tool preserved a small, understandable tool surface. P2-18's
no-op/public-knowledge guard also remains a useful quality bar for future
benchmarks. The review did find documentation drift: the top of
`docs/VSCODE_EXTENSION_PLAN.md` still said M3 was held for release even
though v0.3.0 was live; fixed in this review.

**Next recommendation**: do not start a large new feature until the Glama and
Claude distribution blockers are either resolved or clearly aging out. The
highest-value near-term work is small: keep checking Glama `#support` +
public API, capture VS Code Marketplace install/usage signal at the next
manual check, and consider a focused feasibility spike for `--output-pyc` or
VS Code trace/config workflow polish once the external queue is stable.

**Prior (2026-08-06)**: **VS Code extension P2-2 milestone chain complete (M0/M1/M2 all shipped), plus a same-day expedited bugfix patch.** `pyobfus` 0.5.12 released — pure version-bump/CHANGELOG-promotion for M0 (`pyobfus-trial status --json` / `pyobfus-license status --json`, code already merged 2026-08-04), tagged via OIDC + PEP 740, PyPI confirmed latest. Same session: `vscode-extension` 0.2.0 (M2 — status bar tier indicator, "Generate pyobfus.yaml", right-click "Obfuscate with pyobfus", guided Pro-trial/unlock commands) published to the Marketplace — 24/24 tests green locally against the real CLI, clean `.vsix` package. Along the way, empirically confirmed how to update an *already-listed* Marketplace extension (M1's runbook only covered first-ever publish): a "⋯" overflow menu next to the extension's name → "Update" → upload the new `.vsix` → a transient "verifying `<version>`" state before the public listing flips over — written up in `docs/VSCODE_MARKETPLACE_PUBLISHER_SETUP.md`. Also fixed a small pre-existing gap: the extension's own `CHANGELOG.md` had never split M1's content into a `[0.1.0]` header (M1 and M2 were both sitting under one `[Unreleased]`) — now `[Unreleased]` → `[0.2.0]` → `[0.1.0]`. **Then, hands-on testing of the freshly-shipped 0.2.0 immediately surfaced a real crash** in "Obfuscate with pyobfus": `runJsonCommand` never set an explicit `cwd` for `python -m pyobfus`, which puts cwd first on `sys.path` — an ambient cwd (or sibling) named `pyobfus` shadows the real install as a namespace package. Fixed (`runJsonCommand` now defaults `cwd` to `os.tmpdir()`; the one call site that needs a real project-rooted cwd for `pyobfus.yaml` auto-discovery, `obfuscateFile.ts`, gets a dedicated `cwdForTarget()`) and released same-day as **`vscode-extension` 0.2.1**, jumping the usual spacing gate since it fixes a genuine crash rather than shipping a routine feature. 8 new tests, 32/32 total passing, real CI green. Next PyPI release-spacing gate (per the convention below): eligible **2026-08-08** (2 days after 0.5.12). **M3** (`pyobfus.yaml` IntelliSense) is now **code-complete**, same day as its own scoping: new `pyobfus/config_schema.py` fixes a real core bug found while scoping (`--validate-config` false-warned on `preset` and every Pro field added since v0.5.0) and feeds a JSON Schema (`scripts/generate_vscode_schema.py` → `vscode-extension/schemas/pyobfus.schema.json`) wired into the extension via a declarative `contributes.yamlValidation` entry — simpler than the original design's runtime-API-registration plan, revised mid-implementation after finding `redhat.vscode-yaml`'s docs supported a purely declarative path with no activation-order risk. Held for its own Marketplace gate as v0.3.0; see `docs/VSCODE_EXTENSION_PLAN.md`'s M3 section for the full account.
