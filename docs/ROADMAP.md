...
- [~] **P2-17: Signed obfuscation provenance manifest (Pro/Core)** — **selected 2026-08-01, assigned to Codex** (branch `codex/p2-17-provenance-manifest`, PR pending Claude Code review; see `docs/POST_V0.4_TODO.md` § item 6). A normal pyobfus invocation emits a signed JSON manifest (files obfuscated, config hash, tool version, mapping digest; optional sigstore — not yet a dependency, add only if the manifest's core value proves out without it first). Rides the 2026 SLSA/SBOM/provenance wave, reuses the existing digest pattern in `pyobfus/core/mapping.py`, and is something PyArmor's phone-home-on-build model structurally can't offer (no local-verifiable provenance). _Estimate: 3-5 days_
...
- [~] **P2-19: ML/model-serving preset (`--preset ml`)** — **selected 2026-08-01, assigned to Codex** (branch `codex/p2-19-ml-preset`, PR pending Claude Code review; see `docs/POST_V0.4_TODO.md` § item 6). Protect inference-wrapper code, route model-path / weight-file constants into the Runtime String Vault, surface pickle-safety guidance. Rides the HuggingFace pickle-RCE wave; near-zero architecture cost — follow the existing `preset_fastapi`/`preset_django`/etc. classmethod pattern in `pyobfus/config.py` (registered in the preset-name dict around line 623). _Estimate: 3-5 days_
...
**Last Updated**: 2026-08-01 — P2-18 internal evidence complete: full 5-sample corpus × two model families (Codex + Claude, both subscription-auth, no API accounts), clean cross-model C4 data point on `price_rules`, decision made that a third model family isn't needed. Reviewed results: `docs/LLM_RESISTANCE_PILOT_RESULTS_2026-08-01.md`. Next open decision is feature selection (item 6 in `docs/POST_V0.4_TODO.md` — the original launch-feedback poll gate needs revisiting since real engagement came back thin).
