...
更新时间：2026-09-10（补充发布后干净日复查结论）

## 结论

项目当前的工程基础已经足够支撑扩展分发：Core `0.5.22`、MCP `0.3.12`、VS
...
2026-09-10 的复查把这一判断从「暂时无法证明」变成**已有结论**：0.5.22 发布后的
三个干净日为 `65 / 48 / 31`，单调回落进 20–40 的安静区间，与历次发布尾巴同形，
**没有基线抬升**；MCP 09-07 的 `235` 之后是 `16 / 23`，同样确认为发版与 Glama
重建的自动化流量。截止 09-09 Core 为 `31 / 509 / 1,874`（日/周/月）。VS Code
Marketplace 为 `183 downloads / 7 installs`，Open VSX 为
`228 downloads / 0 reviews`——两者口径不同且不能相加，其中 installs 两天未增长。
即：**已上架的渠道增加了曝光面，但尚未转化出可观测的自然用户**，所以下一阶段的
重点应当是可归因的入口（GitHub Action）而不是继续增加功能或再上架一个目录。
完整历史与口径见 [`DISTRIBUTION_CHANNELS.md`](DISTRIBUTION_CHANNELS.md)。
