...
**已发布**：自定义 runtime 再分发许可证已按审查建议获批并修改，PyPI 也已配置第三个、
项目级的 `pyobfus-runtime` Trusted Publisher（仓库 `zhurong2020/pyobfus`、workflow
...
integration 12、runtime 2 及 Black/Ruff/mypy/MkDocs strict 均通过。已于 09-24 获批准推送
...
- **2026-09-16 下载量复查（数据截止 09-15，不含 09-16 的 0.5.27 发布日）**：Core
  ...
  → `122`(09-13, 0.5.26) → `41`(09-14) → `99`(09-15)。发布日尖峰规律不变
  （123/225/122），随后回落；但 **09-15 的 `99` 高于既往 20–40 安静区间**，是
  0.5.26（09-13）异常长尾还是新基线抬升，需待 09-16 发布日之后 2–3 个干净日
  （09-17 起）才能判定，本次不当作自然增长。月累计 trailing 30 天由 09-10 的
  `1,874` 升至 `2,336`，主因窗口右移纳入更多发布日；剔除尖峰后非发布日基线未见
  持续抬升。09-16 的 0.5.27 发布日数据尚未入 pypistats（约 1 天延迟）。MCP
  09-13/09-14 为 `5 / 9`，仍在安静区间。分渠道口径见
...
## 2026-09-13 · 许可系统可靠性（本轮主线 · 冷启动先读这段）

一位新客户 2026-09-12 报激活失败，查下去不是他的问题：**Pro 联网激活对所有客户
全线失效**，最后一次成功验证停在 `2026-07-08`，近两个月无人察觉。根因是
Cloudflare 边缘拦 `urllib` 默认的 `Python-urllib/*` User-Agent（403 +
`error code: 1010`），请求根本到不了 Worker，客户端把它退化成一句无信息量的
`Access denied`。**实测对照**：只改 UA，`Python-urllib` 被拦，`python-requests`、
浏览器 UA、甚至完全不发 UA 都通。

真正让一次 CDN 策略变更变成两个月收入损失的，是两件设计缺陷：许可系统对付费客户
**fail closed**，而且**没有任何监控**。排查中又查出三件更早的缺陷，都比主故障更久：

- **8 月那位企业客户其实也从未成功激活**（`devices: []` 且无 `last_verified`），
  他只说了 "going well" 并转去问发票，没提激活。**空 devices 是「从未成功验证」的
  指纹，不是无害默认值。**
- **一封客户求助邮件躺了三个月未读未回**（2026-06-10）。他要求重置设备绑定，而
  三个槽位已满被服务端直接拒绝，其中**一格是我们自己写的 `test-device`**。
- **设备指纹把 `platform.release()` 算进哈希**，OS 小版本升级就换身份、本地缓存
  随之失效、CLI 报「没有许可证」、重新注册再烧一格。这正是他抱怨的
  「it keeps thinking I'm different devices」。另外 `uuid.getnode()` 拿不到网卡
  地址时 Python 返回随机数，那样每次运行都是新设备。

还有一个反向缺陷：**吊销从来没生效过**。服务端说 revoked，客户端的离线缓存回退
把它吞掉，照样返回 valid，消息里还原样印着 "revoked"。

### 已完成（四项，均已 commit，未发版未部署）

1. **UA 修复 + 合成监控**（`04788b5`）。验证请求现在自报家门；定时 workflow 用一个
   **不存在的**序列号打生产端点，断言拿回的是 Worker 自己的 JSON——零副作用，不碰
   客户数据、不占设备槽位、不需要真序列号进 CI secrets。已实测正常时绿、复刻旧 UA
   故障时红、端点不可达时红。
2. **fail open / fail closed 各归其位**（`aec5d0c`）。设备标识漂移不再让本地缓存
   失效（HMAC 签名仍然校验并仍然拒绝被改过的缓存）；吊销与过期改为独立异常、越过
   缓存；响应带稳定 `code`，客户端同时认旧的纯文本形式；Worker 不再拒绝第 4 台，
   改为淘汰最久未用的。
3. **`pyobfus-license deactivate` + 管理端重置端点**（`3699b6b`）。释放设备成了客户
   自己的一条命令。服务器不可达时**什么都不改**，包括本地许可——槽位还占着，把能用的
   本地许可也删掉只会让客户更糟。管理端点先验票再查库，所以未授权方无法靠响应
   探知某个序列号是否存在，且用独立 secret 而非 Cloudflare API token。
4. **设备指纹改为持久化随机标识**（`9c4df59`）。`~/.pyobfus/device_id` 跨 OS 升级、
   venv、重建容器都稳定。该文件可复制，但旁边的许可缓存同样可复制，而
...
token 写权限 → 手工改生产 KV → 调回权限」。端点现在有了，但 **KV 仍无定期导出**，
「改动前先导出」目前只是约定不是机制。
...
7.1. ✅ **已随 `0.5.25` 发布（2026-09-12）：包 `__init__.py` re-export 三方名字
   不一致** + 同族的「`import json` 模块绑定被改名」（实现可复现性时发现，
   非那次改动引入；用户拍板先修这个）。
   两趟登记 + 链式解析 + 第三方源不登记 + 包名 alias，9 个新回归测试在未修复树上
   全失败、修复后全过。两文件最小复现与修法见
   [`FEATURE_EXPANSION_RESEARCH_2026-09-12.md`](FEATURE_EXPANSION_RESEARCH_2026-09-12.md)
   §3.3b：`__all__` 里的名字在生成的包里根本不存在；更大 fixture 上调用方的
   `import` 语句未改写而调用点已改写。三种 flag 组合均复现。建议单独排期，
   不要和可复现性改动混在同一个版本。
...
- **下载量复查（数据截止 09-05）**：`pyobfus` 日/周/月 52 / 520 / 1,796；
  ...
  不应以它为依据。
- **下载量再查（2026-09-07，数据截止 09-06）**：`pyobfus` 日/周/月
  **158 / 561 / 1,809**；`pyobfus-mcp` **5 / 185 / 816**。**09-06 是 0.5.22
  发布日**，158 落在既有发布日区间（117–158）内，形状与前五次一致；month 相比
  上次的 1,796 基本持平，week 上升只是 158 进了窗口；MCP 本轮未发版，5 属低位
  非发布日。**这次取数不能结掉上一条挂着的问题**——「09-05 的 52 是否基线抬升」
  需要干净日，而 09-06 被发布污染，干净日自 09-07 起算，数据要等 09-08 之后再看。
  判断不变：仍无有机增长信号。
  - 取数补充：`/recent` 对两个包都会 429。可直接用
    `overall?mirrors=false` 每日序列自算 day/7d/30d——本次用 `pyobfus-mcp`
    做过口径校验，自算值与 `/recent` 实测 `5 / 185 / 816` 完全一致，限流时
    不必干等。
...
- **2026-09-02 第二次周期复查（数据截止 08-31）**：最新 day/week/month 为
  Core `31/376/2,338`、MCP `6/232/1,090`。剔除发布日后，Core 08-25--31
  均值/中位数为 `26.4/27`，与安静基线一致；MCP 为 `13.5/12`，虽略高但样本
  短且紧邻密集发布，不升级为有机增长。GitHub 14 天为 `1,999 clones / 202
  unique`、`214 views / 85 unique`，仍有明显发布尖峰；6 stars、2 forks、0
  open issue/PR，Discussion 无新反馈。数据尚未覆盖 09-01 的 0.5.20/0.3.10，
  待至少覆盖 09-02 后再做该版本专项复查。
...
5. 下载量：✅ 2026-09-02 已完成第二次周期复查（数据截止 08-31）。最新
   rolling day/week/month 为 Core `31/376/2,338`、MCP `6/232/1,090`；Core
   非发布日中位数仍为 `27`，MCP 为 `12`，整体仍由发布尖峰主导，未证明有机
   增长。因数据尚未覆盖 09-01 SEO 发版，待至少覆盖 09-02 后补一次专项复查。
