...
4. **0.5.3 — finish the P2-8 / opacity-config surface** (deferred from 0.5.1; renumbered from 0.5.2, which the Py3.9/3.10 patch took): ✅ **`--period` (run-count counter file) shipped 2026-07-07** (commit `dfe1dbd`; module-top `period_check` guard + runtime `default_counter_path`, `$PYOBFUS_COUNTER_DIR` override, per-module counters; 1027 tests). Still open: `--bind-device` (runtime device-key substitution — **has a binding-model design fork: build-on-target vs supplied-machine-id**, and requires substituting the emitted `_LAYER_KEY`/`_VAULT_KEY_*` literals with a runtime `bind_device_key(current_machine_id(), salt)` derivation without disturbing the shipped opacity/vault ciphertext format) and `--opacity-config` TOML pattern rules (needs name-map coupling so patterns match pre-mangle qualnames). Design notes in `docs/V0.5_RELEASE_PLAN.md`.
