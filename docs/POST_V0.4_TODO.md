...
**Published to PyPI**: 0.5.0 (2026-06-18) → 0.5.1 + pyobfus-mcp 0.3.1 (2026-06-22) → **0.5.2 (2026-06-22, patch)**. Patent gate cleared 2026-06-17 (申请号 202610712171X). Python 3.8 dropped (floor 3.9). Merged to main; nothing is held anymore. Open items, roughly in priority order:
...
4. **0.5.3 — finish the P2-8 / opacity-config surface** (deferred from 0.5.1; renumbered from 0.5.2, which the Py3.9/3.10 patch took): `--bind-device` (runtime device-key substitution), `--period` (run-count counter file), `--opacity-config` TOML pattern rules (needs name-map coupling so patterns match pre-mangle qualnames). Design notes in `docs/V0.5_RELEASE_PLAN.md`.
