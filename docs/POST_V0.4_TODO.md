...
4. **0.5.3 — ✅ FEATURE-COMPLETE on main 2026-07-07 (unreleased; awaiting version bump + tag + publish decision).** All three deferred build-fusion flags shipped to main:
   ...
   **↳ 0.5.4 (next feature release) — deferred follow-ups (recorded 2026-07-07):**
   ...
   - **Doc-sync sweep — surface the LATEST version, retire stale-prominent v0.4 framing** (user-flagged 2026-07-07: the public GitHub README still leads with "New in v0.4.0" while 0.5.x is buried). Concrete gaps:
     - `README.md` `### 🤖 New in v0.4.0 — AI-native features` (~line 56, a top h3) — v0.4 features (preflight / unmap / init / MCP) are now baseline; demote the heading and add a prominent near-top **"What's new (0.5.x)"** callout surfacing the 6 patent-targeted Pro mechanisms + the 0.5.3 flags (currently only a deep h4 at ~line 125, easy to miss).
