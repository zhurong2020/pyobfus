...
| Glama tool-count re-index 0/5 → 7 | **2026-06-05: root-caused + fixed** (commit `bbeefb8`) — `pyobfus_mcp/Dockerfile` pinned crash-on-startup `0.1.1` → Glama introspection got no running server → API `tools:[]` (page's "5" was stale cache). Fixed pin → `0.2.0` + glama.yaml/README 5→7. **Verify**: `curl -s https://glama.ai/api/mcp/v1/servers/zhurong2020/pyobfus \| python3 -c "import sys,json;print(len(json.load(sys.stdin)['tools']))"` → expect 7. **Status 2026-06-05 11:10 UTC: still 0** (pushed ~1.7h prior; Glama not yet rebuilt). Auto-recheck scheduled remote routine `trig_01QA5dyJhcWihr4XLmbw7n4D` fires 2026-06-08 09:00 CST. If still <7 by then: GitHub-login Glama → server-page rebuild, or page "Report Issue". Root-cause memory: `glama_introspection_dockerfile_pin_2026-06-05.md`. | 1-3 days (Glama container rebuild) |
