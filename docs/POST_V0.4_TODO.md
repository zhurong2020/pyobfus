...
4. **0.5.3 — ✅ FEATURE-COMPLETE on main 2026-07-07 (unreleased; awaiting version bump + tag + publish decision).** All three deferred build-fusion flags shipped to main:
   ...
   1033 tests green (+8 for 0.5.3), ruff/format/mypy clean on all new code. **Next: maintainer decides the version bump (0.5.2 → 0.5.3) + tag + PyPI publish; then re-bump the Glama Build-steps pin (see memory `glama_container_build_source`).** Design notes in `docs/V0.5_RELEASE_PLAN.md`.
