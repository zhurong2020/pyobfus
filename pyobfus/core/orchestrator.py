...
class CrossFileOrchestrator:
    ...
    def phase1_scan(self, input_dir: Path) -> GlobalSymbolTable:
        ...
        for file_info in self.files:
            ...
            # Sorted, not raw set order: `exports` is a set, so iterating it
            # directly assigns I0/I1/... in an order that changes with the
            # interpreter's string hash seed. That made the generated bytes
            # differ between two runs of the same input, which in turn made the
            # output digests in --build-report and --provenance-manifest
