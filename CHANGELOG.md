...
- **The same input now produces the same output bytes.** In directory /
  ...
  which symbol became `I0`. Two builds of an unchanged project therefore
  differed, which made the output digests in `--build-report` and
  `--provenance-manifest` impossible for a recipient to re-derive, and weakened
  the incremental cache signature. Export registration now iterates in sorted
  order, and file discovery returns a sorted list instead of raw `rglob` order
  (that order varies by filesystem, so builds could also differ between
  machines). Single-file mode already sorted its names and is unchanged.

  The reproducibility claim is deliberately narrow: it covers name and
  structure selection. `--numeric-obfuscation` and AES string encryption draw
  from a CSPRNG on every build and are **expected** to differ between runs; the
  saved mapping carries a `created_at` timestamp for the same reason, though
  its `marker_id` and name map are stable. A regression test builds the same
  project under two different `PYTHONHASHSEED` values and compares every
  generated byte, the mapping content and the reported digests.
...
- **Generated output no longer embeds the absolute path of the input file.**
  The attribution header wrote `# Original: <absolute path>` verbatim, so every
  shipped file disclosed the build machine's directory layout — and, on a
  typical developer setup, the account name in it. The marker now carries only
  a project-relative POSIX path, falling back to a bare basename when the
  source sits outside the input root and omitting the field when neither is
  available. Reached output through the Pro build-fusion write path, which was
  the only path that emitted the header at all.

- **Community output now actually carries an attribution marker.** The
  single-file path built the header and then discarded it: the non-fusion
  branch regenerated the file from the AST, dropping the annotated string.
  Directory mode never requested one. Both paths now stamp the marker, so the
  free tier's transparent-attribution promise holds in practice rather than
  only in the source.

### Added

- **Versioned, machine-readable build marker.** Generated files begin with a
  `# pyobfus:generated format=1 edition=<community|pro>` block naming the tool
  version and the project-relative source. It is prologue-safe (a shebang stays
  first, a PEP 263 encoding cookie stays within the first two lines) and
  idempotent, so rebuilding never double-stamps. The prefix is deliberately
  distinct from `# pyobfus:obfuscated`, so tooling can tell "this file was
  generated by pyobfus" apart from "a mapping exists to reverse it".

  The marker is transparent attribution — a comment anyone can delete. It is
  not a license check, an authenticity proof, or an anti-piracy measure, and
  removing it is not gated behind Pro.

- **`--community-marker` / `--no-community-marker` and the `community_marker`
  config key** (`auto` | `on` | `off`) for environments that forbid generated
  banners. The CLI flag is tri-state, so it does not clobber a preset's or
  config file's choice when omitted. Marker policy never alters transformation
  semantics; it only decides whether the comment block is emitted. Because it
  changes output bytes it participates in the effective config hash.

- **`output_marker` in the `--dry-run --json` plan and the provenance
  manifest**, reporting `format`, `edition`, `mode` and `emitted`. Additive:
  manifests written before this release still validate. It records build
  provenance and must not be read as proof that an artifact is authentic.
...
integration would have had to scrape colored text — the VS Code extension's
status bar needs a real contract. Text-mode output is unchanged. 12 new
tests. `pyobfus-mcp` unchanged, no Glama Build-steps re-pin needed.
