...
第一反应是 PyArmor。我看了一下，发现两件事不太对：(1) 真正能让反编译变难的能力都在付费 Pro 层，而 PyArmor 免费版的代码量上限对我那个项目刚好不够用（[实测文档](https://github.com/zhurong2020/pyobfus/blob/main/docs/PYARMOR_TRIAL_LIMIT_EXPERIMENT.md)：免费版单文件超过约 940 行就拒绝混淆）。(2) 即使付费用了，工作流也对不上：我那阵子代码大半是 Claude Code 写的，崩溃日志也是贴给 Claude 看的，一个把所有类名改成 `I0` 的混淆器一上线，崩溃栈回来 Claude 只能回一句 *"I have no idea what `I0` or `I2` refer to"*。我亲手装的保护反倒把我天天靠的助手挡在门外。

PyArmor 是给「人读崩溃日志」的时代设计的，保护本来就是单向，没法反向出原始名字。Cython 直接机器码更糟。两者放在 2013、2017 年都说得过去，但调试位上坐着模型的项目里，老假设接不上了。

再加上 Claude Code 当时还很新，我正想试试 vibe coding 能跑多远。从零写一个 Python 混淆器不算大项目（AST 改写 + 名字映射 + CLI 包装，边界清楚），刚好拿来试。

所以我没付钱，花了一个月的晚上跟 Claude Code 一起把工具写出来。围绕一个取舍：保护对外不变，但对自己留一份小到能塞密码管理器的反向映射表。pyobfus 0.4.0，2026-04-22 在 PyPI 发布。既是一个工具，也是 vibe coding 这种工作方式的一次自我验证。
