...
**Cold-start session 第一句话应问 user**：「今天是不是到 2026-08-08 了？到了的话所有排队的东西一起发：pyobfus 下个版本（含 `config_schema.py` 核心 bug 修复）+ vscode-extension v0.3.0（M3 YAML IntelliSense）+ 已写好的 P2-23 对比内容——**验证已在 08-07 做完，直接执行发布机械步骤即可，不用重新跑测试**。」附带常规检查：plugin marketplace 审核（`https://platform.claude.com/plugins/submissions`）查一下有没有新进展，那个"protected_project"笔误要不要顺手改掉（只存在于 Anthropic Console 提交表单文字里，不在仓库任何文件，无法本地修复）；PyPI 下载量隔几天再补个快照（上次 2026-08-07，见上）；Glama 公开 API 工具数是否已追上 0.3.5。
