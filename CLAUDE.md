...
**Cold-start session 第一句话应问 user**：「今天是不是到 2026-08-08 或 08-09 了（0.5.13 之后 1-2 天）？」附带常规检查：**Glama 公开 API 的 `tools` 字段复查一次**（`curl -s "https://glama.ai/api/mcp/v1/servers/zhurong2020/pyobfus" | python3 -c "import json,sys; print(len(json.load(sys.stdin).get('tools', [])))"`，08-07 复查仍是 0）——本地已用 Layer 1(pip install 真实包直接 stdio)+Layer 2(套 Glama 自家 `mcp-proxy@6.4.3`)完整复现排除我方问题，Discord 消息已写好待发（General 频道 @punkpeye，全文见 memory `glama_zero_tools_repro_2026-08-07.md`），**user 明确要求先不发、等这次复查若仍是 0 才正式发**；plugin marketplace 审核（`https://platform.claude.com/plugins/submissions`）查一下有没有新进展，那个"protected_project"笔误要不要顺手改掉（只存在于 Anthropic Console 提交表单文字里，不在仓库任何文件，无法本地修复）；PyPI 下载量隔几天再补个快照（上次 2026-08-07，pyobfus 日181/周1017/月1746、mcp 日48/周442/月593，见 memory）；P2-2 已全部收官，下一个方向待重新扫描/挑选（ROADMAP 里 P2-20 的 RTD 重定向尾巴、P3 长线项都还在，非阻塞）。
