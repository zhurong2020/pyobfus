...
**当前状态**：发明专利初审合格（2026-06-17）→ v0.5.0→0.5.1→0.5.2→0.5.3（PyPI 2026-06~07-07）→ **0.5.4 已发布（2026-07-19，PR #19 vault-key 设备绑定）**→ **issue #22 mypy gate 已关闭（2026-07-22，PR #23）**。dev.to + r/Python showcase + Show HN（<https://news.ycombinator.com/item?id=49130416>）均已发（记录见 `_drafts/launch-v0.5.4/README.md`）→ **CN 长文已发布（2026-08-01）**：<https://www.arong.eu.org/pyobfus-six-mechanisms-now-public/>（findata `publish.py` 全 pipeline 一次跑完，WP #1706 + Discord 已推，微信本篇主动跳过——内容偏技术不适合该渠道，非遗漏）→ **GitHub Discussions 补课（2026-08-01）**：9 个月未回的 Q&A #2 已答（`exclude_names`/`@opacity(Layer.TRANSPARENT)` 用法）并标记 resolved；新发一条覆盖 0.5.0-0.5.4 的 Announcement（#24）补上专利解锁叙事 → **GitHub Insights 首次显示真实流量信号（2026-08-01）**：14 天 165 views/65 unique visitors（Google/Bing/DuckDuckGo 搜索引擎自然来源为主）+ 731 clones/93 unique cloners（含自动化噪音,不能直接当真人数），但 PyPI 下载量还没跟着涨——流量到下载的转化是接下来该盯的指标。mcp 仍 0.3.1（工具面无变化，dep `>=0.5.1` 自动解析到当前版本）。
...
- ✅ **P2-18 两个模型家族的首批真实(非 stub)pilot 都已跑完（2026-08-01）**：Codex CLI/`gpt-5.6-sol`(luhn+billing_auth × C0-C5)先跑,过程中发现并修复一个语料库方法论 bug——`luhn`/`caesar`/`roman` 是公开知名算法,攻击者靠训练常识就能还原,不代表混淆被攻破;`luhn` 对 `--string-encryption`(C2)是 byte-for-byte no-op,曾把 C2 统计悄悄稀释成假 50%(修复见 `conditions.py`/`harness.py` 的 `builds_on`+`skip_reason: noop`)。同一天新增 **`ClaudeCodeCliAttacker`**(`claude -p` 走订阅登录,不需要 Anthropic API 账号,解除了原先卡住第二模型家族的政策性顾虑)并跑了同样的 luhn+billing_auth × C0-C5,过程中又修了两个跟 attacker 无关的既有 plumbing bug——`--json-schema` 不认 schema 文件里的 `$schema` meta key、docker 打分沙箱的 temp dir 默认 0700 权限导致容器内 nobody 用户读不到文件(此前 docker executor 从没被真实跑通过,任何 attacker 走这条路都会中招)。**跨模型信号一致**:两个模型在 `billing_auth`(唯一非公开知识样本)上表现完全一样——C0/C1 全还原、C2/C3/C5 全扛住(0% SRR/100% resistance)。离可发布的公开结果还差:扩大语料库(`caesar`/`price_rules`/`roman` 尚未跑)+ 换一个非公开知识的 C4 目标(`price_rules` 现成可用,两个模型家族在 C4 都还没有干净数据点)。详见 `docs/POST_V0.4_TODO.md` § P2-18。
- ⏭️ **更后续**（完整清单见 `docs/POST_V0.4_TODO.md` 顶部「Current prioritized TODO」）:① launch wave 五渠道(dev.to/r-Python/Show HN/CN 长文/Discussions)全部发完,剩下是**盯真实反馈**——Show HN 评论区(我这边对 news.ycombinator.com 的抓取持续被 429,只能靠你自己看)+ 新 Discussions Announcement(#24)有没有人回 + PyPI 下载量会不会跟上 Insights 里的流量信号 ② P2-18 benchmark 继续扩语料库(caesar/price_rules/roman 未跑)+ 补一个干净的 C4 数据点(price_rules) ③ 下一个新功能选型/开 GitHub 投票,故意等 launch 真实反馈到位再定(`NEXT_FEATURE_DECISION.md`)④ IP 商业化迁移(个人→旎嵘科技)。

**Cold-start session 第一句话应问 user**：「0.5.4 + mcp 0.3.1 已发布、issue #22 mypy gate 已关、P2-18 两个模型家族(Codex+Claude)首批 pilot 已跑完、launch wave 五渠道(dev.to/r-Python/Show HN/CN 长文/GitHub Discussions)全部发完(CN 稿 2026-08-01 已发 arong.eu.org,见上;Discussions 补了 9 个月未回的 Q&A + 一条新公告)、GitHub Insights 首次看到真实搜索引擎流量但 PyPI 下载还没跟涨——继续盯这波流量转化/回 Show HN 评论、扩 P2-18 benchmark 语料库(caesar/price_rules/roman + 干净 C4 数据点)、考虑是否该开 GitHub 功能投票了,还是推进 IP 商业化迁移(个人→旎嵘科技)?」
