...
**Cold-start session 第一句话应问 user**：「今天是不是到 2026-08-06 了？到了的话我就把 M0（trial/license `--json`）发布掉（`0.5.12`，PyPI）+ M2（vscode-extension v0.2.0）发布掉（Marketplace，手工 `.vsix` 上传，流程见 `docs/VSCODE_EXTENSION_PLAN.md`"Marketplace publishing note"）。」M1 已发布，不用再问 Marketplace 账号注册的事。附带常规检查：plugin marketplace 审核（`https://platform.claude.com/plugins/submissions`）查一下有没有新进展，那个"protected_project"笔误要不要顺手改掉。
