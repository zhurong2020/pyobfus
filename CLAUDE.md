...
- ⏭️ **更后续**（完整清单见 `docs/POST_V0.4_TODO.md` 顶部「Current prioritized TODO」）:**当前重心 = P2-18 + 后续功能**（① 扩语料库:caesar/price_rules/roman 未跑 ② 补一个干净的非公开知识 C4 数据点(price_rules 现成可用) ③ 达到 credible public result 后再选下一个功能,候选见 ROADMAP P2-17/P2-19 等）。Launch wave 已收工转被动监测(+7d/+30d checkpoint,不主动推),GitHub 功能投票暂不开(这轮真实互动数据太薄,开了大概率也收不到票)。IP 商业化迁移(个人→旎嵘科技)排在更后面。

**Cold-start session 第一句话应问 user**：「launch wave 已经收工(五渠道全发完,互动数据薄但已决定不再推,详见上方),当前重心是 P2-18 benchmark——继续扩语料库(caesar/price_rules/roman)+ 补干净的 C4 数据点(price_rules),还是已经想好下一个具体功能要做了?」
